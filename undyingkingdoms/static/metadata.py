from undyingkingdoms.models.achievements import Achievement
from undyingkingdoms.models.buildings import Building
from undyingkingdoms.models.armies import Army


metadata_races = ['Human', 'Elf', 'Dwarf', 'Goblin']

metadata_backgrounds = ['Warlord', 'Engineer', 'Merchant', 'Rogue']

kingdom_names = ["Faenoth", "Aldoroth", "Ecthalion"]

infiltration_missions = ['scout military', 'pilfer', 'burn crops', 'sow distrust']

rations_terminology = [(0, "None"), (0.25, "Quarter"), (0.5, "Half"), (1, "Normal"), (2, "Double"), (3, "Triple")]

all_buildings = ['houses', 'fields', 'pastures', 'mills', 'mines', 'forts', 'stables', 'guilds', 'banks']

all_achievements = {
    'land': Achievement(name="land", category="reach_x_amount_in_one_age", sub_category="land",
                        display_title="From Coast to Coast",
                        description="Have your county reach x amount of land in one playthrough.",
                        tier1=175, tier2=250, tier3=300, tier4=400, tier5=500, maximum_tier=5,
                        points_rewarded=5),
    'population': Achievement(name="population", category="reach_x_amount_in_one_age", sub_category="population",
                              display_title="Like Rabbits...",
                              description="Have your county reach x population in one playthrough.",
                              tier1=750, tier2=1500, tier3=2250, tier4=3000, tier5=4000, maximum_tier=5,
                              points_rewarded=5),
    'gold': Achievement(name="gold", category="reach_x_amount_in_one_age", sub_category="gold",
                        display_title="Midas Touch",
                        description="Have your county reach x gold in one playthrough.",
                        tier1=1000, tier2=2000, tier3=3000, tier4=4000, tier5=5000, maximum_tier=5,
                        points_rewarded=5),
    'wood': Achievement(name="wood", category="reach_x_amount_in_one_age", sub_category="wood",
                        display_title="Where Did the Forests Go?",
                        description="Have your county reach x wood in one playthrough.",
                        tier1=500, tier2=1000, tier3=1500, tier4=2000, tier5=2500, maximum_tier=5,
                        points_rewarded=5),
    'iron': Achievement(name="iron", category="reach_x_amount_in_one_age", sub_category="iron",
                        display_title="Heart of Iron",
                        description="Have your county reach x iron in one playthrough.",
                        tier1=500, tier2=1000, tier3=1500, tier4=2000, tier5=2500, maximum_tier=5,
                        points_rewarded=5),
    'dwarf_class_leader': Achievement(name="dwarf_class_leader", category="class_leader", sub_category="dwarf",
                                      display_title="Lord Under the Mountain!",
                                      description="Become the leader of your kingdom while playing as Dwarves.",
                                      maximum_tier=1,
                                      points_rewarded=15),
    'human_class_leader': Achievement(name="human_class_leader", category="class_leader", sub_category="human",
                                      display_title="Return of the King",
                                      description="Become the leader of your kingdom while playing as Humans.",
                                      maximum_tier=1,
                                      points_rewarded=15),
    'elf_class_leader': Achievement(name="elf_class_leader", category="class_leader", sub_category="elf",
                                    display_title="King of the Forest",
                                    description="Become the leader of your kingdom while playing as Elves.",
                                    maximum_tier=1,
                                    points_rewarded=15)
}

# Racial/Class Modifiers (A modifier of 0 means +0%. A modifier of 1 would mean +100%)
birth_rate_modifier = {'Elf': ("Elders", -0.15)}
death_rate_modifier = {}
income_modifier = {'Merchant': ("Silver Tongue", 0.15)}
offensive_power_modifier = {'Warlord': ("Relentless", 0.10)}
infiltration_success_modifier = {'Rogue': ("Master of Disguise", 0.10)}
production_per_worker_modifier = {'Dwarf': ("Dwarven Made", 0.10), 'Engineer': ("", 0.20)}
defense_per_citizen_modifier = {'Elf': ("Citizen Militia", 1.00)}
food_consumed_modifier = {'Dwarf': ("Ravenous", 0.15)}

dwarf_buildings = {'houses': Building(base_name='houses',
                                      class_name='cottage',
                                      class_name_plural='cottages',
                                      total=25,
                                      production_cost=20,
                                      labour_maintenance=0,
                                      gold=35,
                                      wood=25,
                                      output=1,
                                      description='Each {} adds +{} to your birth rate.'),
                   'fields': Building(base_name='fields',
                                      class_name='grain field',
                                      class_name_plural='grain fields',
                                      total=20,
                                      production_cost=20,
                                      labour_maintenance=5,
                                      gold=35,
                                      wood=5,
                                      output=25,
                                      description='Each {} provides enough grain for {} people a day.'),
                   'pastures': Building(base_name='pastures',
                                        class_name='goat farm',
                                        class_name_plural='goat farms',
                                        total=5,
                                        production_cost=20,
                                        labour_maintenance=15,
                                        gold=35,
                                        wood=5,
                                        output=30,
                                        description='Each {} can feed {} people a day, but excess milk is lost.'),
                   'mills': Building(base_name='mills',
                                     class_name='lumber mill',
                                     class_name_plural='lumber mills',
                                     total=3,
                                     production_cost=25,
                                     labour_maintenance=15,
                                     gold=50,
                                     wood=20,
                                     output=3,
                                     description='Each {} produces {} wood a day.'),
                   'mines': Building(base_name='mines',
                                     class_name='iron mine',
                                     class_name_plural='iron mines',
                                     total=1,
                                     production_cost=35,
                                     labour_maintenance=15,
                                     gold=65,
                                     wood=15,
                                     output=3,
                                     description='Each {} produces {} iron a day.'),
                   'forts': Building(base_name='forts',
                                     class_name='stronghold',
                                     class_name_plural='strongholds',
                                     total=0,
                                     production_cost=50,
                                     labour_maintenance=25,
                                     gold=100,
                                     wood=75,
                                     output=6,
                                     description='Dwarf {}s are very effective and add +{}% to your county\'s defence value.'),
                   'stables': Building(base_name='stables',
                                       class_name='pony stables',
                                       class_name_plural='pony stables',
                                       total=0,
                                       production_cost=25,
                                       labour_maintenance=15,
                                       gold=40,
                                       wood=20,
                                       output=4,
                                       description='Each {} lets your army return from battle {}% faster.'),
                   'guilds': Building(base_name='guilds',
                                      class_name='thieves\' den',
                                      class_name_plural='thieves\' dens',
                                      total=0,
                                      production_cost=45,
                                      labour_maintenance=25,
                                      gold=65,
                                      wood=50,
                                      output=1,
                                      description="Each {} gives you {} additional spies to send on missions."),
                   'banks': Building(base_name='banks',
                                     class_name='vault',
                                     class_name_plural='vaults',
                                     total=0,
                                     production_cost=50,
                                     labour_maintenance=5,
                                     gold=75,
                                     wood=25,
                                     output=10,
                                     description="Each {} gives you {} additional gold per day.")
                   }
human_buildings = {'houses': Building(base_name='houses',
                                      class_name='homestead',
                                      class_name_plural='homesteads',
                                      total=25,
                                      production_cost=20,
                                      labour_maintenance=0,
                                      gold=35,
                                      wood=25,
                                      output=1,
                                      description='Each {} adds +{} to your birth rate.'),
                   'fields': Building(base_name='fields',
                                      class_name='wheat field',
                                      class_name_plural='wheat fields',
                                      total=20,
                                      production_cost=20,
                                      labour_maintenance=5,
                                      gold=35,
                                      wood=10,
                                      output=25,
                                      description='Each {} provides enough grain for {} people a day.'),
                   'pastures': Building(base_name='pastures',
                                        class_name='dairy pasture',
                                        class_name_plural='dairy pastures',
                                        total=5,
                                        production_cost=20,
                                        labour_maintenance=15,
                                        gold=35,
                                        wood=10,
                                        output=30,
                                        description='Each {} can feed {} people a day, but excess milk is lost.'),
                   'mills': Building(base_name='mills',
                                     class_name='lumber mill',
                                     class_name_plural='lumber mills',
                                     total=3,
                                     production_cost=25,
                                     labour_maintenance=15,
                                     gold=50,
                                     wood=20,
                                     output=3,
                                     description='Each {} produces {} wood a day.'),
                   'mines': Building(base_name='mines',
                                     class_name='iron mine',
                                     class_name_plural='iron mines',
                                     total=1,
                                     production_cost=35,
                                     labour_maintenance=15,
                                     gold=75,
                                     wood=15,
                                     output=3,
                                     description='Each {} produces {} iron a day.'),
                   'forts': Building(base_name='forts',
                                     class_name='palisade',
                                     class_name_plural='palisades',
                                     total=0,
                                     production_cost=35,
                                     labour_maintenance=10,
                                     gold=75,
                                     wood=35,
                                     output=5,
                                     description='Each {} adds +{}% to your county\'s defence value.'),
                   'stables': Building(base_name='stables',
                                       class_name='stables',
                                       class_name_plural='stables',
                                       total=0,
                                       production_cost=45,
                                       labour_maintenance=25,
                                       gold=60,
                                       wood=25,
                                       output=6,
                                       description='Each {} lets your army return from battle {}% faster.'),
                   'guilds': Building(base_name='guilds',
                                      class_name='thieves\' guild',
                                      class_name_plural='thieves\' guilds',
                                      total=0,
                                      production_cost=45,
                                      labour_maintenance=10,
                                      gold=60,
                                      wood=50,
                                      output=1,
                                      description="Each {} gives you {} additional spies to send on missions."),
                   'banks': Building(base_name='banks',
                                     class_name='bank',
                                     class_name_plural='banks',
                                     total=0,
                                     production_cost=40,
                                     labour_maintenance=5,
                                     gold=50,
                                     wood=20,
                                     output=7,
                                     description="Each {} gives you {} additional gold per day.")
                   }

elf_buildings = {'houses': Building(base_name='houses',
                                    class_name='sanctuary',
                                    class_name_plural='sanctuaries',
                                    total=25,
                                    production_cost=20,
                                    labour_maintenance=0,
                                    gold=40,
                                    wood=25,
                                    output=1,
                                    description='Each {} adds +{} to your birth rate.'),
                 'fields': Building(base_name='fields',
                                    class_name='rice field',
                                    class_name_plural='rice fields',
                                    total=20,
                                    production_cost=20,
                                    labour_maintenance=5,
                                    gold=35,
                                    wood=10,
                                    output=25,
                                    description='Each {} provides enough grain for {} people a day.'),
                 'pastures': Building(base_name='pastures',
                                      class_name='dairy pasture',
                                      class_name_plural='dairy pastures',
                                      total=5,
                                      production_cost=20,
                                      labour_maintenance=15,
                                      gold=35,
                                      wood=10,
                                      output=30,
                                      description='Each {} can feed {} people a day, but excess milk is lost.'),
                 'mills': Building(base_name='mills',
                                   class_name='lumber mill',
                                   class_name_plural='lumber mills',
                                   total=3,
                                   production_cost=25,
                                   labour_maintenance=15,
                                   gold=50,
                                   wood=20,
                                   output=3,
                                   description='Each {} produces {} wood a day.'),
                 'mines': Building(base_name='mines',
                                   class_name='iron mine',
                                   class_name_plural='iron mines',
                                   total=1,
                                   production_cost=35,
                                   labour_maintenance=15,
                                   gold=75,
                                   wood=15,
                                   output=3,
                                   description='Each {} produces {} iron a day.'),
                 'forts': Building(base_name='forts',
                                   class_name='citadel',
                                   class_name_plural='citadels',
                                   total=0,
                                   production_cost=35,
                                   labour_maintenance=10,
                                   gold=75,
                                   wood=35,
                                   output=5,
                                   description='Each {} adds +{}% to your county\'s defence value.'),
                 'stables': Building(base_name='stables',
                                     class_name='stables',
                                     class_name_plural='stables',
                                     total=0,
                                     production_cost=45,
                                     labour_maintenance=25,
                                     gold=60,
                                     wood=25,
                                     output=6,
                                     description='Each {} lets your army return from battle {}% faster.'),
                 'guilds': Building(base_name='guilds',
                                    class_name='thieves\' guild',
                                    class_name_plural='thieves\' guilds',
                                    total=0,
                                    production_cost=45,
                                    labour_maintenance=10,
                                    gold=60,
                                    wood=50,
                                    output=1,
                                    description="Each {} gives you {} additional spies to send on missions."),
                 'banks': Building(base_name='banks',
                                   class_name='exchequer',
                                   class_name_plural='exchequers',
                                   total=0,
                                   production_cost=40,
                                   labour_maintenance=5,
                                   gold=50,
                                   wood=20,
                                   output=7,
                                   description="Each {} gives you {} additional gold per day.")
                 }

goblin_buildings = {'houses': Building(base_name='houses',
                                    class_name='sanctuary',
                                    class_name_plural='sanctuaries',
                                    total=25,
                                    production_cost=20,
                                    labour_maintenance=0,
                                    gold=40,
                                    wood=25,
                                    output=1,
                                    description='Each {} adds +{} to your birth rate.'),
                 'fields': Building(base_name='fields',
                                    class_name='rice field',
                                    class_name_plural='rice fields',
                                    total=20,
                                    production_cost=20,
                                    labour_maintenance=5,
                                    gold=35,
                                    wood=10,
                                    output=25,
                                    description='Each {} provides enough grain for {} people a day.'),
                 'pastures': Building(base_name='pastures',
                                      class_name='dairy pasture',
                                      class_name_plural='dairy pastures',
                                      total=5,
                                      production_cost=20,
                                      labour_maintenance=15,
                                      gold=35,
                                      wood=10,
                                      output=30,
                                      description='Each {} can feed {} people a day, but excess milk is lost.'),
                 'mills': Building(base_name='mills',
                                   class_name='lumber mill',
                                   class_name_plural='lumber mills',
                                   total=3,
                                   production_cost=25,
                                   labour_maintenance=15,
                                   gold=50,
                                   wood=20,
                                   output=3,
                                   description='Each {} produces {} wood a day.'),
                 'mines': Building(base_name='mines',
                                   class_name='iron mine',
                                   class_name_plural='iron mines',
                                   total=1,
                                   production_cost=35,
                                   labour_maintenance=15,
                                   gold=75,
                                   wood=15,
                                   output=3,
                                   description='Each {} produces {} iron a day.'),
                 'forts': Building(base_name='forts',
                                   class_name='citadel',
                                   class_name_plural='citadels',
                                   total=0,
                                   production_cost=35,
                                   labour_maintenance=10,
                                   gold=75,
                                   wood=35,
                                   output=5,
                                   description='Each {} adds +{}% to your county\'s defence value.'),
                 'stables': Building(base_name='stables',
                                     class_name='stables',
                                     class_name_plural='stables',
                                     total=0,
                                     production_cost=45,
                                     labour_maintenance=25,
                                     gold=60,
                                     wood=25,
                                     output=6,
                                     description='Each {} lets your army return from battle {}% faster.'),
                 'guilds': Building(base_name='guilds',
                                    class_name='thieves\' guild',
                                    class_name_plural='thieves\' guilds',
                                    total=0,
                                    production_cost=45,
                                    labour_maintenance=10,
                                    gold=60,
                                    wood=50,
                                    output=1,
                                    description="Each {} gives you {} additional spies to send on missions."),
                 'banks': Building(base_name='banks',
                                   class_name='exchequer',
                                   class_name_plural='exchequers',
                                   total=0,
                                   production_cost=40,
                                   labour_maintenance=5,
                                   gold=50,
                                   wood=20,
                                   output=7,
                                   description="Each {} gives you {} additional gold per day.")
                 }

all_armies = ["peasant", "archer", "soldier", "elite"]

dwarf_armies = {'peasant': Army(base_name='peasant',
                                class_name='miners',
                                total=20,
                                trainable_per_day=25,
                                gold=10,
                                iron=2,
                                wood=1,
                                upkeep=6,
                                attack=2,
                                defence=1,
                                health=2,
                                description='Miners deal extra damage when attacking.'),
                'soldier': Army(base_name='soldier',
                                class_name='axemen',
                                total=10,
                                trainable_per_day=15,
                                gold=15,
                                iron=4,
                                wood=2,
                                upkeep=8,
                                attack=4,
                                defence=2,
                                health=3,
                                description='Axemen are well-rounded soldiers.'),
                'archer': Army(base_name='archer',
                               class_name='riflemen',
                               total=30,
                               trainable_per_day=10,
                               gold=20,
                               iron=4,
                               wood=3,
                               upkeep=8,
                               attack=0,
                               defence=5,
                               health=3,
                               description='Dwarf riflemen are a powerful defensive force.'),
                'elite': Army(base_name='elite',
                              class_name='greybeards',
                              total=0,
                              trainable_per_day=5,
                              gold=30,
                              iron=10,
                              wood=5,
                              upkeep=13,
                              attack=9,
                              defence=6,
                              health=7,
                              description='Greybeards are capable of defending, as well as attacking.')
                }

human_armies = {'peasant': Army(base_name='peasant',
                                class_name='men-at-arms',
                                total=20,
                                trainable_per_day=50,
                                gold=7,
                                iron=1,
                                wood=1,
                                upkeep=3,
                                attack=1,
                                defence=1,
                                health=1,
                                description='Men-at-arms can be trained extremely quickly and have low upkeep.'),
                'soldier': Army(base_name='soldier',
                                class_name='footmen',
                                total=10,
                                trainable_per_day=15,
                                gold=15,
                                iron=4,
                                wood=2,
                                upkeep=8,
                                attack=4,
                                defence=2,
                                health=2,
                                description='Footmen are strong attackers.'),
                'archer': Army(base_name='archer',
                               class_name='crossbowmen',
                               total=30,
                               trainable_per_day=15,
                               gold=20,
                               iron=4,
                               wood=3,
                               upkeep=9,
                               attack=0,
                               defence=5,
                               health=2,
                               description='Crossbowmen are excellent at defending.'),
                'elite': Army(base_name='elite',
                              class_name='knights',
                              total=0,
                              trainable_per_day=5,
                              gold=35,
                              iron=12,
                              wood=4,
                              upkeep=15,
                              attack=12,
                              defence=3,
                              health=8,
                              description='Knights are one of the best attackers.')
                }

elf_armies = {'peasant': Army(base_name='peasant',
                              class_name='defenders',
                              total=20,
                              trainable_per_day=25,
                              gold=9,
                              iron=1,
                              wood=1,
                              upkeep=6,
                              attack=1,
                              defence=2,
                              health=1,
                              description='They excel at repelling enemy armies.'),
              'soldier': Army(base_name='soldier',
                              class_name='rangers',
                              total=10,
                              trainable_per_day=15,
                              gold=20,
                              iron=4,
                              wood=3,
                              upkeep=10,
                              attack=5,
                              defence=2,
                              health=2,
                              description='Excellent offensive troops.'),
              'archer': Army(base_name='archer',
                             class_name='longbowmen',
                             total=30,
                             trainable_per_day=15,
                             gold=20,
                             iron=4,
                             wood=3,
                             upkeep=9,
                             attack=0,
                             defence=5,
                             health=2,
                             description='Longbowmen are very effective at defending your county.'),
              'elite': Army(base_name='elite',
                            class_name='dragonhelm',
                            total=0,
                            trainable_per_day=5,
                            gold=40,
                            iron=12,
                            wood=5,
                            upkeep=15,
                            attack=12,
                            defence=5,
                            health=7,
                            description='Dragonhelms are elite cavalry and incredibly adept at attacking enemy counties.')
              }

goblin_armies = {'peasant': Army(base_name='peasant',
                              class_name='defenders',
                              total=20,
                              trainable_per_day=25,
                              gold=9,
                              iron=1,
                              wood=1,
                              upkeep=6,
                              attack=1,
                              defence=2,
                              health=1,
                              description='They excel at repelling enemy armies.'),
              'soldier': Army(base_name='soldier',
                              class_name='rangers',
                              total=10,
                              trainable_per_day=15,
                              gold=20,
                              iron=4,
                              wood=3,
                              upkeep=10,
                              attack=5,
                              defence=2,
                              health=2,
                              description='Excellent offensive troops.'),
              'archer': Army(base_name='archer',
                             class_name='longbowmen',
                             total=30,
                             trainable_per_day=15,
                             gold=20,
                             iron=4,
                             wood=3,
                             upkeep=9,
                             attack=0,
                             defence=5,
                             health=2,
                             description='Longbowmen are very effective at defending your county.'),
              'elite': Army(base_name='elite',
                            class_name='dragonhelm',
                            total=0,
                            trainable_per_day=5,
                            gold=40,
                            iron=12,
                            wood=5,
                            upkeep=15,
                            attack=12,
                            defence=5,
                            health=7,
                            description='Dragonhelms are elite cavalry and incredibly adept at attacking enemy counties.')
              }

game_descriptions = {"attack": "How much power a unit has when attacking another county.",
                     "defence": "How much power a unit has when defending your county.",
                     "health": "Health affects how many of your units die in battle.",
                     "available_workers": "Any citizen not in the military or employed, is available. These workers "
                                          "will add production to your city which can be used to build new buildings. "
                                          "Unused production generates gold.",
                     "upkeep": "Monthly cost of paying your soldiers' salaries.",
                     "upkeep_daily": "Today's expected salary cost."}
