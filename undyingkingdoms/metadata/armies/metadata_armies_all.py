from undyingkingdoms.models.armies import Army

generic_armies = {
    Army(name='peasant',
         class_name='peasant',
         class_name_plural='peasants',
         total=0,
         trainable_per_day=20,
         gold=5,
         iron=1,
         wood=1,
         upkeep=7,
         category='Not Implemented',
         attack=2,
         defence=1,
         health=1,
         armour_type='Not Implemented',
         description='Unknown',
         ability='None',
         ability_description='None'),
    Army(name='soldier',
         class_name='soldier',
         class_name_plural='soldiers',
         total=0,
         trainable_per_day=10,
         gold=20,
         iron=3,
         wood=3,
         upkeep=20,
         category='Not Implemented',
         attack=6,
         defence=1,
         health=3,
         armour_type='Not Implemented',
         description='Unknown',
         ability='None',
         ability_description='None'),
    Army(name='archer',
         class_name='archer',
         class_name_plural='archers',
         total=20,
         trainable_per_day=10,
         gold=25,
         iron=5,
         wood=5,
         upkeep=25,
         category='Not Implemented',
         attack=0,
         defence=5,
         health=4,
         armour_type='Not Implemented',
         description='Unknown',
         ability='None',
         ability_description='None'),
    Army(name='besieger',
         class_name='besieger',
         class_name_plural='besiegers',
         total=0,
         trainable_per_day=3,
         gold=50,
         iron=25,
         wood=25,
         upkeep=25,
         category='Infantry',
         attack=1,
         defence=0,
         health=10,
         armour_type='Not Implemented',
         description='A siege weapon capable of taking down heavily fortified enemies.',
         ability='Siege',
         ability_description='When attacking, gain +1 attack for each enemy fortification.'),
    Army(name='elite',
         class_name='elite',
         class_name_plural='elites',
         total=0,
         trainable_per_day=5,
         gold=40,
         iron=7,
         wood=7,
         upkeep=40,
         category='Infantry',
         attack=12,
         defence=2,
         health=6,
         armour_type='Not Implemented',
         description='Unknown',
         ability='None',
         ability_description='None'),
    Army(name='monster',
         class_name='monster',
         class_name_plural='monsters',
         total=0,
         trainable_per_day=1,
         gold=200,
         iron=25,
         wood=25,
         upkeep=200,
         category='Not Implemented',
         attack=50,
         defence=10,
         health=25,
         armour_type='Not Implemented',
         description='Unknown',
         ability='None',
         ability_description='None')
}

generic_armies = {
    unit.name: unit
    for unit in generic_armies
}
